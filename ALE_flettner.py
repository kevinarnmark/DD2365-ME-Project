...
res_dir = "flettner_results_ALE_" + repr(des) + "_" + repr(rpm) + "_" + repr(res) + "_" + repr(int(re))
if (path.isdir(res_dir)):
    ...
    ss_plt_f = ss[2]
    ss_nr = ss[3] + 1
    #ss_rot = ss[0] * rpm * 2 * np.pi / 60
    #ss_rot = ss[0] / dt
    #latest_ss = str(max([int(a[-10:-4]) for a in glob.glob(res_dir + "/u*.vtu")]))
    #ss_u1 = open("./u" + "0"*(6-len(latest_ss)) + latest_ss +".vtu").read()
    #latest_ss = str(max([int(a[3:-4]) for a in glob.glob("./u*.xml")]))
    #ss_u1 = "./u" + "0"*(6-len(latest_ss)) + latest_ss +".xml"
    #ss_p1 = "./p" + "0"*(6-len(latest_ss)) + latest_ss +".xml"
    ss_u1 = res_dir + "/u.xml"
    ss_p1 = res_dir + "/p.xml"
    #rmtree(res_dir)
    #mkdir(res_dir)
...
if (saved_state):
    ss_rot = ((ss_time) / dt) * omega
    ss_w = Expression(('x[0]*o0-x[1]*o1 - x[0]','x[0]*o1+x[1]*o0 - x[1]'), rc=rc, dt=dt, o0=cos(ss_rot), o1=sin(ss_rot), xc=xc, yc=yc, element = V.ufl_element())
    ALE.move(mesh, ss_w)
...
print("Flettner Simulation Time:", TIME.time() - start_time, "Using:", des, rpm, res)